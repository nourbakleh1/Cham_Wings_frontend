  ...
  return (
    ...
    <div className='flex justify-center items-center flex-wrap absolute top-1 left-0'> 
      {seats_plan2?.map((el,idx)=>{
        return (
          <div key={el.id}>
          <input type='checkbox' id={`seat2 ${idx}`} checked={el.checked =="yes"} disabled={el.status == "Occupied" ? true :false} className='seat hidden' onChange={()=>setSelectedSeats2(()=>
          {
          ...
          <label  htmlFor={`seat2 ${idx}`}>
          <FontAwesomeIcon icon={faGlassWater} checked={el.checked =="yes"} className={`${el.seat_number.endsWith("d")? `pl-5` : ``} icon text-[20px] p-[7px] py-2 ${el.class_id == "business"? `bg-primary_color/20`:"bg-secoundary_color/20"} ${el.status =="Occupied"?`text-red_color/50`:`text-green_color/50`}`}/>       
          </label>
