...
import { get_Occupied_seats } from '../../Redux/ApiSlices/reservationSlice';

const Reservation_seats = () => {
  const dispatch=useDispatch();
  const {occupied_seats}=useSelector(state=>state.reservation);

  console.log("occupied_seats",occupied_seats)
    ...
    useEffect(()=>{
      if(window.localStorage.getItem("Sel_flight")){
        const x= JSON.parse(window.localStorage.getItem("Sel_flight"));
        setFlight_details_dep({id:x[0]?.flightId,type:x[0]?.classType});
        setFlight_details_ret({id:x[1]?.flightId,type:x[1]?.classType});
      }
    ...
    console.log(departure_flight_selected?.schedule_time_id)

    const return_flight_selected=flights?.departure_flights?.find((el)=>{
      return el?.flight_id == flight_details_ret?.id
    })
    
    useEffect(()=>{
      dispatch(get_Occupied_seats(departure_flight_selected?.schedule_time_id));
      // there is selected seats ?????????
    },[departure_flight_selected]);
  ...
  return (
    ...
    <div className='p-5 bg-off_white w-[200px] sm:w-[250px]  lg:w-[350px] text-center shadow-lg shadow-primary_color'>
      ...
      <div className='p-0 sm:p-5 shadow-lg shadow-primary_color z-[100] relative'>
        ...
        <span className='text-[15px] text-center text-white p-1  sm:border-2 border-solid border-secoundary_color'>{`${return_flight_selected?.arrival_airport_code} to ${return_flight_selected?.departure_airport_code}`}</span>
        <div className='flex flex-wrap sm:flex-nowrap shadow-sm shadow-white_color'>
        ...
        {return_flight_selected &&<FontAwesomeIcon icon={faArrowsTurnToDots}  onClick={()=>setChangeSectors(!changeSectors)} className='text-[30px] rounded-2xl shadow-lg shadow-secoundary_color ml-2 text-white p-3 bg-primary_color absolute bottom-[-100px] right-[40%]'/>
        }
      </div>
      ...
      {
        changeSectors?<Airplane_seats occupied_seats={occupied_seats} flight_details_dep={flight_details_dep} selectedSeats1={selectedSeats1} setSelectedSeats1={setSelectedSeats1} selectedSeats1_name={selectedSeats1_name} setSelectedSeats1_name={setSelectedSeats1_name} />:
        <Airplane_seats2 flight_details_ret={flight_details_ret} selectedSeats2={selectedSeats2} setSelectedSeats2={setSelectedSeats2} selectedSeats2_name={selectedSeats2_name} setSelectedSeats2_name={setSelectedSeats2_name}/>
